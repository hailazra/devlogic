...
local function sendEmbed(info, origin)
    ...
    local DISCORD_CUSTOM_EMOJIS = {
    -- Format setelah upload emoji ke Discord server: <:nama_emoji:ID>
    ...
    -- Function untuk menggunakan Discord custom emoji
local function discordEmoji(emojiKey, fallback)
    return DISCORD_CUSTOM_EMOJIS[emojiKey] or fallback or "🔸"
end

-- Contoh penggunaan dengan Discord custom emoji:
local function sendEmbedWithDiscordEmoji(info, origin)

    -- UPDATED: Enhanced embed with new data
    ocal embed = {
        title = (info.shiny and "✨ " or discordEmoji("fish", "🎣") .. " ") .. "New Catch ",
        description = string.format("**%s Player:** %s", 
            discordEmoji("player", "👤"), 
            hide(LocalPlayer.Name)
        ),
        color = info.shiny and 0xFFD700 or 0x87CEEB,
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
        footer = { text = ".devlogic | Fish-It Notifier" },
        fields = {
            { 
                name = discordEmoji("fish", "🐟") .. " Fish Name",
                value = box(fishName), 
                inline = false 
            },
            { 
                name = discordEmoji("weight", "⚖️") .. " Weight",
                value = box(toKg(info.weight)), 
                inline = true 
            },
            { 
                name = discordEmoji("chance", "🎲") .. " Chance",
                value = box(fmtChanceOneInFromNumber(info.chance)), 
                inline = true 
            },
            { 
                name = discordEmoji("rarity", "💎") .. " Rarity",
                value = box(getTierName(info.tier)), 
                inline = true 
            },
            { 
                name = discordEmoji("mutation", "🧬") .. " Mutation",
                value = box(formatVariant(info)), 
                inline = false 
            },
        }
    }
